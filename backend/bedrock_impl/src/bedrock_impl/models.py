...
#   timestamp: 2025-09-01T23:28:04+00:00
...
class ErrorResponse(BaseModel):
    status: float = Field(
        ..., description='The HTTP status code of the response', examples=[401]
    )
    error: str = Field(
        ...,
    ...
    message: str = Field(
        ...
        examples=['The provided token is invalid.'],
    )
...
class Conversation(BaseModel):
    session_id: str
    user_id: str
    created_at: datetime = Field(..., examples=['2024-08-23T13:28:00Z'])
    last_updated: datetime = Field(..., examples=['2024-08-23T13:28:00Z'])
...
class RAGResponse(BaseModel):
    ...
    response_parts: Optional[List[ResponsePart]] = Field(
        None, description='A list of response parts with their retrieved sources'
    )
